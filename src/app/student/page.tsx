...
export default function StudentDashboardPage() {
  ...
  const fetchStudentDetails = useCallback(
    async (address: string) => {
      try {
        const studentIdFromAddress = await blockchainService.getStudentId(
          address
        );
  ...
  useEffect(() => {
    const initWallet = async () => {
      try {
        ...
        setError(
          "Failed to connect to blockchain. Please make sure your wallet is connected."
        );
      }
    };
  ...
  const handleGetDetails = async () => {
    ...
    try {
      ...
      setError(
        "Failed to get student details. " + (err.message || "Please try again.")
      );
    } finally {
      setFetching(false);
    }
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-yellow-700">
                      No student record found for your address. Click the button
                      below to check if your university has registered you.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {records.length === 0 ? (
                      <tr>
                        ...
                        >
                          No records found. Records will appear here when
                          universities issue them to you.
                        </td>
                      </tr>
                    ) : (
                      records.map((record) => (
                    ...
                    {sharedRecords.length === 0 ? (
                      <tr>
                        <td
                          ...
                          className="px-6 py-4 text-center text-sm text-gray-500"
                        >
                          No shared records found. Records will appear here when
                          other students share them with you.
                        </td>
                      </tr>
                    ) : (
